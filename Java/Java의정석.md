...
- 접근 제어자가 `private` 또는 `default`인 멤버들은 상속은 되지만 자손 클래스로부터의 접근이 제한된다
- 공통된 코드가 사용될 때 모든 클래스의 코드를 수정하는 것 보다 공통 조상의 클래스를 수정할 수 있게되어 코드의 중복이 줄어든다
- 자손 클래스의 인스턴스를 생성하면 조상 클래스의 멤버와 자손 클래스의 멤버가 합쳐진 하나의 인스턴스가 생성된다
- 두 클래스의 관계가 '~은 ~이다'가 성립한다면 상속관계를 고려

### 클래스간의 포함관계

- 클래스를 재사용하는 방법
- 한 클래스의 멤버변수로 다른 클래스 타입의 참조변수를 선언
- 두 클래스의 관계가 '~은 ~을 가지고 있다'가 성립한다면 포함관계를 고려
